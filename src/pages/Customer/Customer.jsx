...
const Customer = () => {
  ...
  const currentItems =
    CustomerInformation &&
    CustomerInformation.slice(indexOfFirstItem, indexOfLastItem);
  ...
  useEffect(() => {
    const customerApi = async () => {
      try {
        ...
        dispatch(getCustomer(response.data.customer));
      } catch (err) {
        console.log(err);
      }
    };
  ...
  const handleAllChecked = () => {
    const ids = CustomerInformation.map((item) => item.userId);
    const allChecked = ids.reduce((acc, id) => {
      acc[id] = checkBox ? false : true;
      return acc;
    }, {});
  ...
  const totalPages =
    CustomerInformation && Math.ceil(CustomerInformation.length / 10);
  ...
  const handleShowDrapdown = (e, index) => {
    e.stopPropagation();
  ...
  const handleIndividualDelete = async (userId) => {
    try {
      ...
      const response = await axios.delete(
        `http://localhost:4000/customer/deleteCustomer/${encodedUserId}`
      );
  ...
  const handleDeleteAll = async () => {
    const leadsToDelete = CustomerInformation.filter((customer) => {
    ...
    try {
      const response = await axios.delete(
        "http://localhost:4000/customer/bundleCustomersDelete",
        {
          data: {
  ...
  const handleShowView = (e, userId) => {
    e.stopPropagation();
    setShowView(true);
    const dataToShow = CustomerInformation.find(
      (data) => data.userId === userId
    );
    setShowCustomer(dataToShow);
  };
  return (
    <div className="w-full flex mt-16  justify-end ">
      ...
      >
        ...
        <section className="w-full bg-white rounded-xl">
          <div className="flex items-center justify-end py-3 gap-x-3 px-3">
            {CustomerInformation && CustomerInformation.length ? (
              <>
                <button
                  type="button"
                  disabled={!checkBox}
                    ...
                    {currentItems.map((person, index) => (
                      <tr key={index} className=" text-xs">
                        ...
                        <td className="pt-5 text-center ">
                          <Link
                            onClick={(e) => {
                              handleShowView(e, person.userId);
                            }}
                        ...
                        <td className="flex items-center  relative pt-5  text-sm">
                          ...
                          <svg
                            onClick={(e) => handleShowDrapdown(e, index)}
                            width="12"
                            height="12"
                            className=" right-1"
                            viewBox="0 0 15 14"
                          ...
                          {showDropdown[index] && (
                            <ul
                              ...
                              className="absolute left-32 text-xs roboto-medium -top-7 w-36 px-2 py-2 rounded-lg text-white bg-gray-400"
                            >
                              ...
                              <li className="my-2 bg-gray-300 rounded-md p-2">
                                <Link
                                  to={`/customer/editCustomer/${person.userId.replace(
                                    "#",
                                    ""
                                  )}`}
                                >
                              ...
                              <li
                                ...
                                className="my-2 bg-gray-300 hover:cursor-pointer rounded-md p-2"
                              >
                                Delete
                              </li>
                            </ul>
        ...
        <div className=" fixed overflow-y-scroll pb-7 w-1/4 px-7 poppins-medium rounded-l-md h-[100vh] right-0 top-0 z-50   bg-[#1B3C6D]">
          <div className="">
            ...
            <section className="px-4 text-sm text-white">
              ...
              <p>
                Status
                <span className="text-xs text-gray-300 mx-2"> -NA-</span>
              </p>
              <p className="my-1">
                Stage
                <span className="text-xs text-gray-300  mx-2">
              ...
              <p className="text-sm  my-1 whitespace-nowrap">
                Start Date:
                <span className="text-xs whitespace-nowrap text-gray-300 mx-2">
                  {showCustomer.startDate.split("T")[0]}
                </span>
              </p>
              <p className="text-sm  my-1 whitespace-nowrap">
                End Date:
                <span className="text-xs whitespace-nowrap text-gray-300 mx-2">
                  {showCustomer.endDate.split("T")[0]}
                </span>
              </p>
              <p className="text-sm  my-1 whitespace-nowrap">
                No of Adults:
                <span className="text-xs whitespace-nowrap text-gray-300 mx-2">
                  {showCustomer.noOfAdults}
                </span>
              </p>
              <p className="text-sm  my-1 whitespace-nowrap">
                No of Children:
                <span className="text-xs whitespace-nowrap text-gray-300 mx-2">
                  {showCustomer.noOfChildren}
                </span>
              </p>
              <p className="text-sm  my-1 whitespace-nowrap">
                Billing:
                <span className="text-xs whitespace-nowrap text-gray-300 mx-2">
                  {showCustomer.billingAmount}
                </span>
              </p>
              <p className="text-sm  my-1 whitespace-nowrap">
                Paid:
      ...
      <div id="filterEl" className="hidden">
        <CustomerFilter handleHideFilter={handleHideFilter} />
      </div>
    </div>
  );
};
